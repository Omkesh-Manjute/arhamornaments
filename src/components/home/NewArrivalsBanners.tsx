...
const NewArrivalsBanners: React.FC = () => {
  return (
    <section className="px-0 md:px-8 py-4 md:py-16 bg-offwhite flex flex-col gap-2 md:gap-8 max-w-7xl mx-auto">
      ...
      <div className="relative w-full aspect-[4/3] md:aspect-[21/9] overflow-hidden md:rounded-2xl group shadow-sm">
        <img 
        ...
        <div className="absolute inset-0 p-6 md:p-16 flex flex-col justify-between">
          <div className="max-w-md">
            <div className="relative inline-block mb-2 text-white">
              <Sparkles className="absolute -top-4 -left-4 w-4 h-4 md:w-6 md:h-6 text-gold animate-pulse" />
              <h2 className="text-4xl md:text-6xl font-heading font-bold tracking-widest uppercase leading-none drop-shadow-lg">
                <span className="text-gold">New</span> <br/> Arrival
              </h2>
              <Sparkles className="absolute -bottom-2 -right-6 w-3 h-3 md:w-5 md:h-5 text-gold animate-pulse delay-300" />
            </div>
          </div>
          <div className="self-end md:self-end mt-auto">
            <Link to="/products" className="inline-flex items-center justify-center bg-white/10 backdrop-blur-md border border-white/30 text-white px-5 md:px-8 py-2 md:py-3 rounded-full font-bold uppercase tracking-widest text-[10px] md:text-xs hover:bg-white hover:text-black transition-colors shadow-[0_0_15px_rgba(255,255,255,0.2)]">
              View Collection
            </Link>
          </div>
        </div>
      ...
      <div className="relative w-full aspect-[4/3] md:aspect-[21/9] overflow-hidden md:rounded-2xl group shadow-sm">
        ...
        <div className="absolute inset-0 p-6 md:p-16 flex flex-col justify-center">
          <div className="max-w-md">
            <div className="flex items-center gap-2 mb-2">
              <div className="h-[1px] w-8 bg-gold"></div>
              <p className="text-gold font-serif italic text-2xl md:text-4xl">18CT</p>
              <div className="h-[1px] w-8 bg-gold"></div>
            </div>
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-white leading-tight mb-2 drop-shadow-md">
              Jewellery <br/> <span className="text-[#e8c39e]">Ready Stock</span>
            </h2>
            <div className="flex items-center gap-4 mt-6">
              <Link to="/products?category=necklaces" className="inline-flex items-center gap-2 bg-gradient-to-r from-[#d4af37] to-[#b5952f] text-white px-6 py-3 rounded font-bold uppercase tracking-widest text-[10px] md:text-xs hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] transition-all">
      ...
      <div className="relative w-full aspect-[4/3] md:aspect-[21/9] overflow-hidden md:rounded-2xl group shadow-sm">
        ...
        <div className="absolute inset-0 p-6 md:p-16 flex flex-col justify-center">
          <div className="max-w-md">
            <h2 className="text-4xl md:text-6xl font-heading font-bold text-[#4a3b32] leading-tight mb-1 uppercase tracking-wider drop-shadow-sm">
              Mix <br/> Rajkot
            </h2>
            <p className="text-[#a67c52] font-heading font-semibold text-xl md:text-3xl mt-2 tracking-widest uppercase">
              22CT Jewellery
            </p>
          </div>
        </div>
